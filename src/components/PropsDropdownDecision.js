...
const PropsDropdownDecision = (props) => {

    //const [decisions, setDecisions] = useState(props.dataDecision);
    ...
    const addDecision = () => {
        ...
        if (newDecision.trim() && !props.dataDecision.find(item => item.name === newDecision.trim())) {
            let newId = props.dataDecision.length + 1;

            // Memory            
            //setDecisions([...decisions, { id: newId, name: newDecision }]);
            props.joinNewDecision({ id: newId, name: newDecision });
            ...
            setNewDecision('');
            console.log(props.dataDecision);
        }
    };
...
export default React.memo(PropsDropdownDecision);